...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non DMC unit input in lnReqAmount, we generate the URI with DMC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::DMC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
